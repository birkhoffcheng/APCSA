 ...
 * Calculate acceleration/gravity on each planet.
 * @author Birkhoff Cheng
 * @version 0.0.1
 * PMR:
 * Basically, I think making programs modular is good. For such reason, I am separating functions in different programs I made into separate files so I can just copy and paste them into the class where I need them and use them.
 */
...
public class Gravity
{
	...
	{
		double gc = 6.67408 * Math.pow(10,-11);
		double[] gravity = new double[radius.length];
		for (int i = 0; i < radius.length; i++)
		{
			mass[i] /= 1000000.0;
			gravity[i] = gc * mass[i] / (radius[i] * radius[i]);
		}
		return gravity;
	}
	...
	public static void printResults(String[] name, double[] radius, double[] mass, double gravity[])
	{
		...
		for (int i = 0; i < radius.length; i++)
		{
			System.out.printf("%10s%15.1f%10.2e%10.2f\n", name[i], radius[i] * (double)2.0, mass[i] * 1000000.0, gravity[i]);
		}
		return;
	}
	...
	{
		PrintWriter file = new PrintWriter(new File("gravity.txt"));
